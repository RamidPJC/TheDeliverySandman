...
public class Minimap : MonoBehaviour
{
    ...
    [Header("RoutePlotting")]
    [SerializeField] private UILineRenderer lineRenderer;

    private Transform _playerWorldLocation;
    private Transform _currentWorldDestination;
    ...
    public void RegisterPlayer(GameObject player)
    {
        ...
        _playerWorldLocation = player.transform;

        player.GetComponent<MissionSystem>().OnGetMissionAccepedHandler += SetNewDestination;

        IMinimapAgent agent = player.GetComponent<IMinimapAgent>();
    ...
    {
        MapAgentGraphics agentGraphics = Instantiate(_agentGraphicsPrefab, _iconsRoot, true);
    ...
    private void SetNewDestination(Transform destination)
    {
        if (destination)
        {
            _currentWorldDestination = destination;
            _pathRoutine = StartCoroutine(UpdatePathRoutine());
        }
        else
        {
            ...
            if (_currentWorldDestination)
            {
                _currentWorldDestination = null;
    ...
    private void UpdateCurrentPath()
    {
        ...
        if (NavMesh.CalculatePath(_playerWorldLocation.position, _currentWorldDestination.position, NavMesh.AllAreas, _currentPath))
        {
            for (int i = 0; i < _currentPath.corners.Length; i++)
            {
                lineRenderer.AddPoint(_mapScale * GetRemappedOffset() + _playerRect.anchoredPosition +
                    MapUtils.GetRemappedRectPosition(_worldMapRect, _currentUiRect, _currentPath.corners[i].FromXZ()));
            }
        }
    }
